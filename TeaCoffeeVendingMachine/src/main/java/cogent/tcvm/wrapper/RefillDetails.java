		...
		for (Refill refill : refillList) {
			...
			if (refillRowMap.containsKey(name)) {
				...
				if (refill.getDate().equals(today)) {
					refillRowMap.put(name,
							new RefillRow(name, quantity + refillRowOld.getAllDays(),
									quantity + refillRowOld.getToday(), unitQuantity + refillRowOld.getUnitQuantity(),
									unitType));
				} else {
					refillRowMap.put(name, new RefillRow(name, quantity + refillRowOld.getAllDays(),
							refillRowOld.getToday(), unitQuantity + refillRowOld.getUnitQuantity(), unitType));
				}
			} else {
				if (refill.getDate().equals(today)) {
					refillRowMap.put(name, new RefillRow(name, quantity, quantity, unitQuantity, unitType));
				} else {
					refillRowMap.put(name, new RefillRow(name, quantity, 0, unitQuantity, unitType));
				}
			}
		}
